...
public class Echo3DService : MonoBehaviour
{
    ...
    private const string src = "UnitySDK";
    ...
    public static string GetCustomQueryURL(string apiKey, string secKey = "", string customQuery = "")
    {
        string serverURL = Echo3DGlobals.endpointURL + "/query?key=" + apiKey + "&secKey=" + secKey + "&src=" + Echo3DGlobals.src + "&" + customQuery;
        return serverURL;
    }
    ...
    public void DownloadAndInstantiate(Entry entry, string serverURL, GameObject targetParent)
    {
        // Get hologram type
        ...
        if (hologramType.Equals(Hologram.hologramType.MODEL_HOLOGRAM))
        {
            ...
            if (modelHologram.getFilename().EndsWith(".glb"))
            {
                ...
                StartCoroutine(InstantiateModel(entry, filenames, importOptions, targetParent, serverURL));
            }
            else
            {
                // Download model files and then instantiate
                StartCoroutine(DownloadFiles(entry, serverURL, filenames, fileStorageIDs, importOptions, targetParent));
            }
    ...
    public IEnumerator WebsocketClient(Echo3DHologram target)
    {
        ...
        {
            // Parse new entry
            Entry entry = ParseEntryToDB(JSON.Parse(message), ref target.hologramData);
            // Download and instantiate content
            DownloadAndInstantiate(entry, target.customQuery, target.gameObject);
        });
        WClient.On(WClient.EventType.DELETE_ENTRY.ToString(), (string message) =>
        {
            ...
            foreach (CustomBehaviour cb in FindObjectsOfType<CustomBehaviour>())
            {
                ...
                {
                    // Remove entry
                    target.hologramData.getEntries().Remove(cb.entry);
                    // Destroy game object
        ...
        WClient.On(WClient.EventType.DATA_POST_ALL.ToString(), (string message) =>
        {
            ...
            // Find entry
            foreach (Entry entry in target.hologramData.getEntries())
            {
                // Update or add key
    ...
    IEnumerator InstantiateModel(Entry entry, List<string> filenames, ImportOptions importOptions, GameObject targetParent, string serverURL)
    {
    ...
    IEnumerator DownloadFiles(Entry entry, string serverURL, List<string> filenames, List<string> fileStorageIDs, ImportOptions importOptions, GameObject targetParent)
    {
